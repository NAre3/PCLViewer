...
{
	// 取得點雲路徑
	QString path = QFileDialog::getOpenFileName(this,"選取點雲文件",".//","點雲文件(*.pcd);;所有文件(*.*)");
	// 讀取點雲數據
	QFile file(path);
	...
	if (file.open(QIODevice::ReadOnly | QIODevice::Text))
	{
		...
		while (!file.atEnd())
		{
			...
			if (strList.size() != 3)
			{
				QMessageBox::warning(this, "警告", "點雲格式錯誤");
				break;
			}
...
// 進行高度渲染
void PCLViewer::setHeightRamp(int dir, double height)
{
	//清空點雲
	viewer->removeAllPointClouds();
	...
	for (double i = min_range - 1; i < max_range + height;)
	{
		...
		pass.setFilterLimits(i, i + height);	//設置過濾範圍
